...
export default function KataCard({ category, name, price, imageUrl, bgColor }) {
  return (
    <div className={`flex-shrink-0 m-6 relative overflow-hidden bg-${bgColor}-500 rounded-lg max-w-xs shadow-lg`}>
      ...
      <div className="relative text-white px-6 pb-6 mt-6">
        ...
        <div className="flex justify-between">
          ...
          <span className={`block bg-white rounded-full text-${bgColor}-500 text-xs font-bold px-3 py-2 leading-none flex items-center`}>{`Kyu: ${price}`}</span>
        </div>
      </div>
    </div>
  );
