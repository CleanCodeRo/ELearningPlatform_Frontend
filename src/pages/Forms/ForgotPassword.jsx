...
const ForgotPassword = () => {
  ...
  const sendEmail = () => {
    ...
    fetch(
      ...
      .then(res =>
         navigate(`/resetPassword?token=${res.response}&email=${emailRef.current.value}`))
      .catch((error) => {
  ...
  return (
    <div id="wholePageHolder"
      ...
      <div id="formLogin" className="relative w-[24rem] flex flex-col items-center px-8 py-5 h-fit rounded-xl bg-white bg-clip-border text-gray-700 shadow-md">
        ...
        <button ref={loginButtonRef} id="loginButton" className=" bg-generalColors-dark-blue text-white rounded-full py-4 w-full" onClick={sendEmail}>SEND RECOVERY</button>
